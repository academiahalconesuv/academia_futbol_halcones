...
// Configuración de multer para almacenar archivos en memoria (Correcto para Railway)
const storage = multer.memoryStorage();
const upload = multer({
  storage,
  limits: { fileSize: MAX_FILE_SIZE }, // **Límite de 2MB**
  fileFilter: (req, file, cb) => {
    // Permitimos PDF, JPG y PNG (combinando ambas lógicas)
...
// **Ruta para subir un comprobante con la lógica corregida**
...
  try {
    const { id } = req.params;
    const { tipo_comprobante } = req.body; // <-- LÓGICA AÑADIDA
...
    }
    ...
    // Validación extra para el tipo de comprobante
    if (!tipo_comprobante) {
        return res.send(`
            <script>
                alert("Error: El tipo de comprobante es requerido.");
                window.history.back();
            </script>
        `);
    }

    // Insertar el archivo en la base de datos (Consulta Corregida)
    await pool.query(
      'INSERT INTO comprobantes_pago (alumno_id, nombre_archivo, archivo_data, estado, fecha_subida, tipo_comprobante) VALUES ($1, $2, $3, $4, NOW(), $5)',
      [id, req.file.originalname, req.file.buffer, 'pendiente', tipo_comprobante]
    );
...
// **Ruta para descargar un comprobante desde la BD (Mantenida de Producción)**
...
// **Ruta para eliminar un comprobante (Mantenida de Producción)**
...
// **Ruta para validar/rechazar un comprobante (Mantenida de Producción)**
...
// **Ruta para obtener el último comprobante de un alumno (Mantenida de Producción)**
